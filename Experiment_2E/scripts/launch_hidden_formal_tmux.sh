...
SESSION=${SESSION:-exp2e_hidden_formal}
LOG=${LOG:-/data2/hjk/logs/experiment_2e/hidden_formal.log}
...
tmux new-session -d -s "${SESSION}" \
  "cd '${PROJECT_ROOT}' && MANIFEST='${MANIFEST}' CKPT_DIR='${CKPT_DIR}' ROLLOUT_DIR='${ROLLOUT_DIR}' bash scripts/run_hidden_formal.sh 2>&1 | tee '${LOG}'"
