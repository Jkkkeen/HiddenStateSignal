...
**Architecture:** A focused Bash launcher creates a unique scratch root, links the formal checkpoint read-only, starts one-second GPU telemetry, and runs each candidate sequentially with identical frozen GRPO settings. Ray and Python multiprocessing sockets use separate short, candidate-unique paths under `/data2/hjk/r` and `/data2/hjk/t` to remain below the Linux AF_UNIX 107-byte limit after their own path components are appended. A Python summarizer parses completed-step metrics and telemetry into a machine-readable audit and applies the frozen stability/usefulness rules.
...
- Each candidate uses `total_epochs=6` so a batch-size change does not make the
