...
**Architecture:** A focused Bash launcher creates a unique scratch root, links the formal checkpoint read-only, starts one-second GPU telemetry, and runs each candidate sequentially with identical frozen GRPO settings. Ray sockets use a separate short, candidate-unique path under `/data2/hjk/r` to remain below the Linux AF_UNIX 107-byte limit after Ray appends its session and socket components. A Python summarizer parses completed-step metrics and telemetry into a machine-readable audit and applies the frozen stability/usefulness rules.
