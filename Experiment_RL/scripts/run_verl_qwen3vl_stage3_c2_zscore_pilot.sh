#!/usr/bin/env bash
# RL02 Stage 3 C2-long-zscore pilot: answer reward plus group-zscored option-logit gain.
...
REWARD_PATH=${REWARD_PATH:-${PROJECT_ROOT}/scripts/mathverse_qwen3vl_option_gain_reward.py}
REWARD_MANAGER_PATH=${REWARD_MANAGER_PATH:-${PROJECT_ROOT}/scripts/qwen3vl_c2_zscore_reward_manager.py}

SEED=${SEED:-42}
RUN_NAME=${RUN_NAME:-qwen3vl8b_stage3_c2_zscore_pilot_seed${SEED}}
...
OPTION_GAIN_ZSCORE_CLIP=${OPTION_GAIN_ZSCORE_CLIP:-2.0}
OPTION_GAIN_CLIP=${OPTION_GAIN_CLIP:-0.5}
OPTION_GAIN_RESPONSE_FRACS=${OPTION_GAIN_RESPONSE_FRACS:-0.0,0.25,0.50,0.90}
OPTION_GAIN_REWARD_START_INDEX=${OPTION_GAIN_REWARD_START_INDEX:-1}
OPTION_GAIN_MAX_RESPONSE_CHARS=${OPTION_GAIN_MAX_RESPONSE_CHARS:-4096}
OPTION_GAIN_DTYPE=${OPTION_GAIN_DTYPE:-bfloat16}
OPTION_GAIN_ATTN_IMPLEMENTATION=${OPTION_GAIN_ATTN_IMPLEMENTATION:-sdpa}
...
export PYTHONPATH="${PROJECT_ROOT}/scripts:${VERL_ROOT}:${PYTHONPATH:-}"
export OPTION_GAIN_MODEL_PATH="${MODEL_PATH}"
export OPTION_GAIN_LAMBDA
export OPTION_GAIN_ZSCORE_CLIP
export OPTION_GAIN_CLIP
export OPTION_GAIN_RESPONSE_FRACS
export OPTION_GAIN_REWARD_START_INDEX
export OPTION_GAIN_MAX_RESPONSE_CHARS
export OPTION_GAIN_DTYPE
export OPTION_GAIN_ATTN_IMPLEMENTATION
...
free -h
nvidia-smi
ls -lh "${TRAIN_FILE}" "${TEST_FILE}" "${REWARD_PATH}" "${REWARD_MANAGER_PATH}"
...
echo "OPTION_GAIN_MAX_RESPONSE_CHARS=${OPTION_GAIN_MAX_RESPONSE_CHARS}"
...
bash examples/grpo_trainer/run_qwen3_vl_8b_fsdp.sh \
    ...
    reward.reward_manager.name=Qwen3VLC2ZscoreRewardManager \
...
status=$?
echo QWEN3VL_STAGE3_C2_ZSCORE_PILOT_EXIT_CODE=${status}
free -h
nvidia-smi
df -h / /data2
exit ${status}
