...
def compute_score(
    ...
    **_: Any,
    ...
    backend = os.environ.get("OPTION_GAIN_BACKEND", "hf_probe").strip().lower()
    ...
    result = {
        ...
        "option_probe_deferred": float(probe_deferred),
        "option_probe_elapsed_s": float(probe_elapsed_s),
        "option_probe_response_chars": float(response_chars),
    }
    if isinstance(extra_info, dict):
        for key in ("question_id", "uid", "index"):
            value = extra_info.get(key)
            if value is not None and str(value) != "":
                result[key] = str(value)
        source_index = extra_info.get("source_index")
        if source_index is not None:
            result["source_index"] = float(source_index)
