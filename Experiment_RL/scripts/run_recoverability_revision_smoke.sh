...
RUN_NAME=${RUN_NAME:-recoverability_discovery_smoke_v1}
...
"${ENV_ROOT}/bin/python" scripts/run_recoverability_revision_vllm.py \
  --input "${INPUT_PATH}" \
  --model "${MODEL_PATH}" \
...
status=$?
echo "RECOVERABILITY_REVISION_EXIT_CODE=${status}"
df -h / /data2
nvidia-smi
exit "${status}"
