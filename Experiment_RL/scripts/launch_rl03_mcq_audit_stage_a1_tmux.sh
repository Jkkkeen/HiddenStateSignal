...
SESSION_NAME=${SESSION_NAME:-rl03_stage_a1_v2_full}
...
if [[ -e "${RUN_ROOT}/exit_code.txt" || -e "${RUN_ROOT}/scores/run_contract.json" ]]; then
  echo "Stage A1 run already exists: ${RUN_ROOT}" >&2
  exit 3
fi
...
bash scripts/run_rl03_mcq_audit_stage_a1.sh > '${LOG_PATH}' 2>&1"
