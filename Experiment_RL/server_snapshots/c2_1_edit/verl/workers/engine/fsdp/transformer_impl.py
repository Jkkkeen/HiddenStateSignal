...
def _c2_actor_probe_config(micro_batch: TensorDict):
    label_token_ids = tu.get_non_tensor_data(data=micro_batch, key="c2_option_label_token_ids", default=None)
    probe_fracs = tu.get_non_tensor_data(data=micro_batch, key="c2_option_probe_fracs", default=None)
    if label_token_ids is None or probe_fracs is None:
        return None
    label_token_ids = [int(item) for item in label_token_ids]
    probe_fracs = [float(item) for item in probe_fracs]
    if len(label_token_ids) != 4 or not probe_fracs:
        return None
    return label_token_ids, probe_fracs


def _c2_response_lengths(response_mask: torch.Tensor) -> list[int]:
    if hasattr(response_mask, "offsets"):
        return [int(item) for item in response_mask.offsets().diff().detach().cpu().tolist()]
    return [int(item) for item in response_mask.to(torch.int64).sum(dim=1).detach().cpu().tolist()]


def _c2_actor_option_logits_from_rmpad(
    logits_rmpad: torch.Tensor,
    input_ids: torch.Tensor,
    response_mask: torch.Tensor,
    label_token_ids: list[int],
    probe_fracs: list[float],
) -> torch.Tensor:
    from qwen3vl_actor_option_probe import probe_positions_for_lengths

    total_lengths = [int(item) for item in input_ids.offsets().diff().detach().cpu().tolist()]
    response_lengths = _c2_response_lengths(response_mask)
    positions = probe_positions_for_lengths(total_lengths, response_lengths, probe_fracs)

    label_ids = torch.tensor(label_token_ids, dtype=torch.long, device=logits_rmpad.device)
    offsets = input_ids.offsets()[:-1].to(device=logits_rmpad.device, dtype=torch.long)
    option_logits = torch.full(
        (len(positions), len(probe_fracs), len(label_token_ids)),
        float("nan"),
        device=logits_rmpad.device,
        dtype=torch.float32,
    )
    for row_idx, row_positions in enumerate(positions):
        row_pos = torch.tensor(row_positions, dtype=torch.long, device=logits_rmpad.device) + offsets[row_idx]
        option_logits[row_idx] = logits_rmpad.index_select(0, row_pos).index_select(1, label_ids).float()
...
class FSDPEngineWithLMHead(FSDPEngine):
    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict, logits_processor_func):
        ...
        c2_probe_config = _c2_actor_probe_config(micro_batch)
        ...
        if use_remove_padding:
            ...
            if use_fused_kernels:
                ...
                log_probs = None
                if c2_probe_config is not None and not self.use_ulysses_sp and "response_mask" in micro_batch.keys():
                    label_token_ids, probe_fracs = c2_probe_config
                    model_output["c2_actor_option_logits"] = _c2_actor_option_logits_from_rmpad(
