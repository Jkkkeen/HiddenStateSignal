...
from verl.utils import build_multimodal_processor_inputs, hf_processor, hf_tokenizer, normalize_token_ids
...
class PPOTrainer(ABC):
    ...
    def step(self, metrics: dict, timing_raw: dict) -> KVBatchMeta:
        ...
        # 4. [OPTIONAL] compute C2.1 option-logit gain with the existing rollout vLLM server.
        if self._c2_vllm_option_gain_enabled():
            with marked_timer("c2_vllm_wake", timing_raw, color="yellow"):
                self.checkpoint_manager.wake_up_replicas()
            try:
                with marked_timer("c2_vllm_probe", timing_raw, color="yellow"):
                    batch = self._compute_c2_vllm_option_gain(batch, metrics=metrics)
            finally:
                with marked_timer("c2_vllm_sleep", timing_raw, color="yellow"):
                    self.checkpoint_manager.sleep_replicas()
    ...
    def _compute_reward_colocate(self, batch: KVBatchMeta, metrics: dict | None = None) -> KVBatchMeta:
        ...
        fields = ["prompts", "responses", "raw_prompt", "extra_fields"]
        data = tq.kv_batch_get(keys=batch.keys, partition_id=batch.partition_id, select_fields=fields)
        prior_extra_fields = data.pop("extra_fields", None)
        ...
        if prior_extra_fields is not None and "extra_fields" in write_back:
            try:
                reward_manager_cls = resolve_reward_manager_cls(self.config)
                merge_extra_fields = getattr(reward_manager_cls, "merge_v1_extra_fields", None)
                if merge_extra_fields is not None:
                    write_back["extra_fields"] = merge_extra_fields(prior_extra_fields, write_back["extra_fields"])
            except Exception as exc:
                print(f"[qwen3vl_c2_zscore_v1] extra_fields merge skipped: {type(exc).__name__}: {exc}", flush=True)
        tq.kv_batch_put(
            keys=batch.keys,
    ...
    @staticmethod
    def _c2_vllm_option_gain_enabled() -> bool:
    ...
    @staticmethod
    def _c2_prompt_text(raw_prompt, extra_info: dict | None) -> str:
        extra_info = extra_info if isinstance(extra_info, dict) else {}
        text = extra_info.get("prompt_text") or extra_info.get("question") or ""
        if text:
            return str(text).replace("<image>", "").strip()

        parts: list[str] = []
        messages = raw_prompt if isinstance(raw_prompt, (list, tuple)) else [raw_prompt]
    ...
    @staticmethod
    def _c2_image_paths(extra_info: dict | None, images_value) -> list[str]:
        extra_info = extra_info if isinstance(extra_info, dict) else {}
        paths: list[str] = []
        ...
        if isinstance(raw_images, (list, tuple)):
            paths.extend(str(item) for item in raw_images if item)
        elif raw_images:
            paths.append(str(raw_images))
        return list(dict.fromkeys(paths))

    def _c2_label_token_ids(self) -> dict[str, int]:
        cached = getattr(self, "_c2_cached_label_token_ids", None)
        ...
        for label in ("A", "B", "C", "D"):
            ...
            if len(token_ids) != 1:
                raise ValueError(f"C2.1 option label {label!r} is not one token: {token_ids}")
    ...
    def _c2_build_probe_prompt(self, prompt_text: str, image_paths: list[str], response_prefix: str, suffix: str, label: str):
        ...
        if image_paths:
            ...
            pil_images = [Image.open(image_paths[0]).convert("RGB")]
        content.append({"type": "text", "text": prompt_text})
        messages = [
            {"role": "user", "content": content},
            {"role": "assistant", "content": f"{response_prefix}{suffix}{label}"},
        ]
        ...
        if self.processor is not None:
            ...
            prompt_ids = normalize_token_ids(self.tokenizer(text, add_special_tokens=False)["input_ids"])
        return prompt_ids, pil_images

    async def _c2_probe_one_rollout(
        self,
        *,
        ...
        from mathverse_qwen3vl_option_gain_reward import (
        ...
        fracs = parse_float_list(os.getenv("OPTION_GAIN_RESPONSE_FRACS", "0.0,0.25,0.50,0.90"))
        suffix = os.getenv("OPTION_GAIN_PROBE_SUFFIX", DEFAULT_PROBE_SUFFIX)
        ...
        result = {
            ...
            "option_probe_elapsed_s": float(time.perf_counter() - probe_started),
            "option_probe_response_chars": float(len(str(response_text or ""))),
        }
        expected_margin_count = len(fracs)
    ...
    async def _c2_probe_batch_async(self, rows: list[dict]) -> list[dict[str, float]]:
        ...
        for row in rows:
            try:
                results.append(
                    await self._c2_probe_one_rollout(client=client, label_token_ids=label_token_ids, **row)
                )
            except Exception as exc:
                print(f"[qwen3vl_c2_1_vllm_probe] probe failed for {row.get('batch_key')}: {type(exc).__name__}: {exc}", flush=True)
                ...
                failed = {
                    ...
                    "option_probe_failed": 1.0,
                    "option_probe_deferred": 0.0,
                    "option_probe_elapsed_s": 0.0,
                    "option_probe_response_chars": float(len(str(row.get("response_text") or ""))),
                }
                for i in range(expected_margin_count):
                    failed[f"option_margin_{i}"] = float("nan")
                results.append(failed)
        return results

    def _compute_c2_vllm_option_gain(self, batch: KVBatchMeta, metrics: dict | None = None) -> KVBatchMeta:
        fields = ["responses", "raw_prompt", "reward_model", "extra_fields", "images", "extra_info"]
        data = tq.kv_batch_get(keys=batch.keys, partition_id=batch.partition_id, select_fields=fields)
        response_lengths = data["responses"].offsets().diff()
        responses = data["responses"].to_padded_tensor(padding=self.tokenizer.pad_token_id)
        rows: list[dict] = []
        raw_prompts = self._c2_to_list(data.get("raw_prompt"), len(batch), None)
        reward_models = self._c2_to_list(data.get("reward_model"), len(batch), {})
        extra_infos = self._c2_to_list(data.get("extra_info"), len(batch), {})
        images_values = self._c2_to_list(data.get("images"), len(batch), None)
        extra_fields = self._c2_to_list(data.get("extra_fields"), len(batch), {})

        for i, key in enumerate(batch.keys):
            valid_response_ids = responses[i, : response_lengths[i]]
            ...
            rows.append(
                {
                    ...
                    "images_value": images_values[i],
                }
            )

        results = asyncio.run(self._c2_probe_batch_async(rows))
        updated_extra = []
        for extra_field, result in zip(extra_fields, results, strict=True):
        ...
        tq.kv_batch_put(
            keys=batch.keys,
            partition_id=batch.partition_id,
            fields=tu.get_tensordict({"extra_fields": updated_array}),
        )
    ...
    def _compute_advantage(self, batch: KVBatchMeta, metrics: dict) -> KVBatchMeta:
        ...
        if extra_fields is not None:
            reward_manager_cls = resolve_reward_manager_cls(self.config)
            postprocess_v1_batch = getattr(reward_manager_cls, "postprocess_v1_batch", None)
            if postprocess_v1_batch is not None:
                scores, extra_fields, c2_metrics = postprocess_v1_batch(batch.keys, extra_fields)
                response_lengths = response_mask.offsets().diff()
                padded_rm_scores = data["rm_scores"].to_padded_tensor(padding=0.0)
                for row_idx, score in enumerate(scores):
                    padded_rm_scores[row_idx].zero_()
                    padded_rm_scores[row_idx, response_lengths[row_idx] - 1] = float(score)
                data["rm_scores"] = response_to_nested(padded_rm_scores, response_mask)
                tq.kv_batch_put(
                    keys=batch.keys,
                    partition_id=batch.partition_id,
                    fields=tu.get_tensordict({"rm_scores": data["rm_scores"], "extra_fields": extra_fields}),
                )
                metrics.update(c2_metrics)
                print("[qwen3vl_c2_zscore_v1] diagnostics", c2_metrics, flush=True)
